...
namespace Nyamkani
{
  ...
  class sick_nav350: public rclcpp::Node
  {
    ...
    public:
      ...
      { 
          scan_pub = this->create_publisher<sensor_msgs::msg::LaserScan>(SCAN_TOPIC, 20);
          odom_pub = this->create_publisher<nav_msgs::msg::Odometry>(ODOM_TOPIC, 50); //rclcpp::QoS(rclcpp::KeepLast(10))

          // Call on_timer function every second
          using namespace std::chrono_literals;
          get_data_callback = this->create_wall_timer(125ms, std::bind(&sick_nav350::Get_Data_From_Nav350, this));

          odom_broadcasters = std::make_unique<tf2_ros::TransformBroadcaster>(*this);    

          Initialization();

      }
      ~sick_nav350(){Uninitailize();}
    ...
    private:
      ...
      rclcpp::TimerBase::SharedPtr get_data_callback{nullptr};
      ...
      tf2::Transform mobile_base_current_tf;
      tf2::Transform mobile_base_prev_tf;
    ...
    private:
      ...
      SickToolbox::SickNav350& Get_Nav350()
      {
  ...
  int sick_nav350::Initialization()
  {
    try
    {
      init_param();
...
{ 
  ...
  {
    sick_nav350_publisher->Process();
    rclcpp::spin_some(sick_nav350_publisher);
    loop_rate.sleep();
  }
